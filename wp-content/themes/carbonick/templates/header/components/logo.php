...
if (!class_exists('Carbonick_Get_Logo')) {
	class Carbonick_Get_Logo{
		...
		public function init($location, $menu, $custom_logo, $custom_logo_height){
			...
			$header_logo_src =  empty($custom_logo) ? Carbonick_Theme_Helper::get_option('header_logo') : $custom_logo;
			...
			$logo_mobile_src =  Carbonick_Theme_Helper::get_option('logo_mobile'.$menu);
			...
			$logo_height_custom = Carbonick_Theme_Helper::get_option('logo_height_custom');
			$logo_height = Carbonick_Theme_Helper::get_option('logo_height');
			...
			$mobile_logo_height_custom = Carbonick_Theme_Helper::get_option('mobile_logo'.$menu.'_height_custom');
			$mobile_logo_height = Carbonick_Theme_Helper::get_option('mobile_logo'.$menu.'_height');
				...
				<a href='<?php echo esc_url(home_url('/')) ?>'>
					...
					switch (true) {
						...
						if (!empty($header_logo_src)) {
							?>
							<img class="default_logo" src="<?php echo esc_url($header_logo_src); ?>" alt="<?php echo esc_attr($def_img_alt); ?>" <?php echo Carbonick_Theme_Helper::render_html($logo_height_style);?>>
							<?php
						} else {
							?>
							<h1 class="logo-name">
						...
						?>
						<img class="logo-mobile" src="<?php echo esc_url($logo_mobile_src);?>" alt="<?php echo esc_attr($mobile_img_alt); ?>" <?php echo Carbonick_Theme_Helper::render_html($mobile_height_style);?>>
						<?php
						break;
