...
if (! class_exists('Carbonick_Get_Header')) {
	class Carbonick_Get_Header
	{
		...
		public function header_vars() {
			...
			* @access public
			*/
			$this->header_type = Carbonick_Theme_Helper::get_option('header_type');
			...
			if ($this->header_type === 'custom') {
				...
				if (!empty($header_page_select)) {
					...
					if (class_exists('SitePress')) {
						$wpml_header = icl_object_id($this->header_page_select_id, 'header', false, ICL_LANGUAGE_CODE);
			...
			*/
			...
			if (Carbonick_Theme_Helper::options_compare('header_sticky','mb_customize_header_layout','custom') == '1') {
				$header_sticky_page_select = Carbonick_Theme_Helper::get_option('header_sticky_page_select');
			...
			if (class_exists( 'RWMB_Loader' ) && $this->id !== 0) {
				...
				if ($customize_header == 'custom') {
					...
					if ($custom_sticky_header !== 'default') {
						...
						if (class_exists('SitePress')) {
							$wpml_sticky_header = icl_object_id( $this->header_sticky_page_select_id, 'header', false, ICL_LANGUAGE_CODE);
							$this->header_sticky_page_select_id = !empty($wpml_sticky_header) ? $wpml_sticky_header : $this->header_sticky_page_select_id;
						}
					}
				}
			}
		...
		public function init()
		{
			...
			//hide if 404 page
			$page_not_found = Carbonick_Theme_Helper::get_option('404_show_header');
		...
		*/
		...
		{
			$header_shadow = Carbonick_Theme_Helper::get_option( 'header_shadow');
			$header_on_bg = Carbonick_Theme_Helper::get_option('header_on_bg');
			$header_class = '';
		...
		*/
		...
		public function header_bar_editor($location = null,$position = null)
		{
			...
			*/
			...
			${'header_'.$position.'_editor'} = Carbonick_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor');
		...
		*/
		...
		public function header_bar_delimiter($k = null)
		{
			...
			*/
			...
			$height = Carbonick_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_height');
			$width = Carbonick_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_width');

			$bg_color = Carbonick_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_bg');

			$margin = Carbonick_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_margin');
			...
			if ($this->html_render === 'sticky') {
				$custom_sticky = Carbonick_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom');
				if (!empty($custom_sticky)) {
					$bg_color = Carbonick_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color');
					$height  = Carbonick_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_height');
				}
			}
		...
		*/
		...
		public function header_bar_button($k = null)
		{
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title');

			$link = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link');

			$target = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target');

			$size = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size');

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn);

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn);

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn);

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border'.$options_btn);
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn);
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn);
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';

			$border_radius = Carbonick_Theme_Helper::get_option('bottom_header_button'.$get_number.'_radius');
			switch ($size) {
			...
			$button_css_id =  uniqid( "carbonick_button_" );
			...
			// Register css
			if (!empty($styles)) {
				Carbonick_Theme_Helper::enqueue_css($styles);
			}
			...
			echo '<div class="header_button">';
				echo '<div class="wrapper">';
					...
					?>
					<div <?php echo Carbonick_Theme_Helper::render_html($this->get_render_attribute_string( 'wrapper' )); ?>>
						<a <?php echo Carbonick_Theme_Helper::render_html($this->get_render_attribute_string( 'button' )); ?>>
		...
		public function render_text($settings) {
			...
			?>
			<span <?php echo Carbonick_Theme_Helper::render_html($this->get_render_attribute_string( 'content-wrapper' )); ?>>
				<span <?php echo Carbonick_Theme_Helper::render_html($this->get_render_attribute_string( 'text' )); ?>><?php echo Carbonick_Theme_Helper::render_html($settings['text']); ?></span>
			</span>
			<?php
		}
		...
		public function get_render_attribute_string( $element) {
			...
			return Carbonick_Theme_Helper::render_html_attributes( $this->render_attributes[ $element ] );
		}
		...
		*/
		...
		public function header_bar_spacer($location = null, $key = null) {
			...
			*/
			...
			$spacer = Carbonick_Theme_Helper::get_option($location.'_header_spacer'.$get_number);
		...
		*/
		...
		public function header_default( $section = 'bottom') {
			...
			$header_layout = Carbonick_Theme_Helper::get_option($section.'_header_layout');
			$lavalamp_active = Carbonick_Theme_Helper::get_option('lavalamp_active');
		...
		private function row_width_class($s = '_middle', $section)
		{
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = Carbonick_Theme_Helper::get_option('header'.$s.'_full_width');
				...
				case 'sticky':
					$width_container = Carbonick_Theme_Helper::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
						$class = "fullwidth-wrapper";
					} else {
						$class = 'wgl-container';
		...
		private function row_style_color($s = '_middle', $section)
		{
			...
			switch ($this->html_render) {
				case 'bottom':
				case 'sticky':
					$header_background = Carbonick_Theme_Helper::get_option('header'.$s.'_background');
					$header_background_image = Carbonick_Theme_Helper::get_option('header'.$s.'_background_image');
					$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';

					$header_color = Carbonick_Theme_Helper::get_option('header'.$s.'_color');
					$header_bottom_border = Carbonick_Theme_Helper::get_option('header'.$s.'_bottom_border');
					$header_border_height = Carbonick_Theme_Helper::get_option('header'.$s.'_border_height');
					$header_border_height = $header_border_height['height'];
					$header_bottom_border_color = Carbonick_Theme_Helper::get_option('header'.$s.'_bottom_border_color');
					...
					$customize_width = Carbonick_Theme_Helper::get_option('header'.$s.'_max_width_custom');

					if ($customize_width == '1') {
						$max_width = Carbonick_Theme_Helper::get_option( 'header'.$s.'_max_width');
		...
		private function row_style_height($s = '_middle', $section, $width = false)
		{
			...
			*/
			...
			$header_mobile_height = Carbonick_Theme_Helper::get_option('header_mobile_height')['height'] ?? false;

			$header_height = Carbonick_Theme_Helper::get_option('header'.$s.'_height')['height'] ?? false;
			...
			switch ($this->html_render) {
				case 'mobile':
					if (!empty($header_mobile_height)) {
						$style = 'height:'.(int)$header_mobile_height.'px;';
						if (!empty($width)) {
							$style .= 'width:'.(int)$header_mobile_height.'px;';
						}
						$style = ' style="'.$style.'"';
		...
		protected function side_panel_style_icon()
		{
			...
			$sticky_icon_switcher = Carbonick_Theme_Helper::get_option('bottom_header_side_panel_sticky_custom');
			...
			$icon_background = Carbonick_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_background');
			$icon_color = Carbonick_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_color');
		...
		private function column_class($s = '_middle', $section, $area)
		{
			...
			* @access private
			*/
			$dispay = Carbonick_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_display');
			$v_align = Carbonick_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_vert');
			$h_align = Carbonick_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_horz');
		...
		*/
		public function build_header_mobile_menu()
		{
			...
			$sub_menu_position = Carbonick_Theme_Helper::get_option('mobile_position');
		...
		*/
		public static function get_logo($location, $menu = false) {
			new Carbonick_Get_Logo( $location, $menu );
		}
		/**
		* Get Header Search
		*
		...
		*/
		...
		{
			$description = esc_html__('Type To Search', 'carbonick');
			...
			$customize = Carbonick_Theme_Helper::get_option('bottom_header_item_search_custom'.$options_btn);
			$customize = empty($customize) ? 'def' : 'color';

			$text_color = Carbonick_Theme_Helper::get_option('bottom_header_item_search_color_txt'.$options_btn);
			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$text_color_hover = Carbonick_Theme_Helper::get_option('bottom_header_item_search_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';


			$search_css_id =  uniqid( "carbonick_search_" );
			...
			echo '<div class="header_search'.esc_attr($search_class).'"'.( $this->row_style_height($location, $section) ).'>';
				echo
					'<div ', $this->get_render_attribute_string( 'search' ), '>',
		...
		public function login_in($location, $section) {
			...
			echo "<div class='login-in woocommerce'".$this->row_style_height($location, $section).">";
				...
				if (is_user_logged_in()) {
					...
					echo "<a class='login-in_link-logout' href='".esc_url( $link_logout )."'>".esc_html__('Logout', 'carbonick')."</a>";
				} else {
					echo "<a class='login-in_link' href='".esc_url_raw( $url )."'>".esc_html__('Login', 'carbonick')."</a>";
				}
		...
		*/
		...
		public function icon_cart()
		{
			...
			$customize = Carbonick_Theme_Helper::get_option('bottom_header_cart_custom'.$options_btn);
			$customize = empty($customize) ? 'def' : 'color';

			$text_color = Carbonick_Theme_Helper::get_option('bottom_header_cart_color_txt'.$options_btn);
			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$text_color_hover = Carbonick_Theme_Helper::get_option('bottom_header_cart_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$cart_css_id =  uniqid( "carbonick_woo_" );
			...
			$this->add_render_attribute( 'cart', 'title', esc_attr__( 'Click to open Shopping Cart', 'carbonick' ) );
			...
			?>
				<a <?php echo Carbonick_Theme_Helper::render_html( $this->get_render_attribute_string( 'cart' ) ) ;?> >
					<span class='woo_mini-count'><?php echo ((WC()->cart->cart_contents_count > 0) ?  '<span>' . esc_html( WC()->cart->cart_contents_count ) .'</span>' : '') ?></span>
				</a>
			<?php
			return ob_get_clean();
		}
