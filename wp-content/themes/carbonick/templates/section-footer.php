...
if (!class_exists('Carbonick_Footer_Area')) {
	/**
	 * Footer area
	 *
	 *
	 * @class		Carbonick_Footer_Area
	 * @version		1.0
	 * @category	Class
	 * @author		WebGeniusLab
	 */
	...
	class Carbonick_Footer_Area
	{
		...
		function __construct()
		{
			...
			$footer_bg_color = Carbonick_Theme_Helper::get_option('footer_bg_color');
			// copyright option
			$copyright_switch = Carbonick_Theme_Helper::get_option('copyright_switch');

			// add global variables
			$this->footer_full_width = Carbonick_Theme_Helper::get_option('footer_full_width');
			...
			$style .= Carbonick_Theme_Helper::bg_render('footer', 'mb_footer_switch', 'on');
			$style = !empty($style) ? ' style="' . esc_attr($style) . '"' : '';


			//hide if 404 page
			$page_not_found = Carbonick_Theme_Helper::get_option('404_show_footer');
	        ...
	        */
			if ($footer_switch || $copyright_switch) {
				...
				if ($footer_switch) {
					$footer_content_type = Carbonick_Theme_Helper::options_compare('footer_content_type', 'mb_footer_switch', 'on');
		...
		{
			$wave_switch = Carbonick_Theme_Helper::options_compare('footer_add_wave', 'mb_footer_switch', 'on');
			if ($wave_switch) {

				$wave_height = Carbonick_Theme_Helper::get_option('footer_wave_height');
				...
				echo "<div class='carbonick_wave_footer' style='height:", (int) $wave_height, "px;'>",
		...
		private function get_footer_vars($optn_1 = null)
		{
			...
			// Get options	
			$footer_spacing = Carbonick_Theme_Helper::options_compare('footer_spacing', 'mb_footer_switch', 'on');
			$footer_border = Carbonick_Theme_Helper::options_compare('footer_add_border', 'mb_footer_switch', 'on');
			$footer_border_color = Carbonick_Theme_Helper::options_compare('footer_border_color', 'mb_footer_switch', 'on');

			// Only for widgets in footer
			if ($optn_1 == 'widgets') {
				$footer_options['widget_columns'] = Carbonick_Theme_Helper::get_option('widget_columns');
				$footer_options['widget_columns_2'] = Carbonick_Theme_Helper::get_option('widget_columns_2');
				$footer_options['widget_columns_3'] = Carbonick_Theme_Helper::get_option('widget_columns_3');
				$footer_align = Carbonick_Theme_Helper::get_option('footer_align');

				//footer container class
				$footer_options['footer_class'] = ' align-' . esc_attr($footer_align);
			}else{
				$footer_options['footer_class'] = ' wgl-elementor-footer';
			}
...
?>
			...
			<?php
							...
							private function main_footer_get_page()
							{
								...
								echo "<div class='footer_top-area".esc_attr($footer_class)."' " . $footer_border_style . ">";
								...
								$footer_page_select = Carbonick_Theme_Helper::options_compare('footer_page_select', 'mb_footer_switch', 'on');
								...
								if (!empty($footer_page_select)) {
									...
									if (class_exists('SitePress') && function_exists('icl_object_id')) {
                                        $wpml_footer = icl_object_id($footer_page_select_id, 'footer', false, ICL_LANGUAGE_CODE);
                                        $footer_page_select_id = !empty($wpml_footer) ? $wpml_footer : $footer_page_select_id;
									}
							...
							{
								// Get options
								$copyright_spacing = Carbonick_Theme_Helper::options_compare('copyright_spacing', 'mb_copyright_switch', 'on');
							...
							{
								$bg_color = Carbonick_Theme_Helper::options_compare('copyright_bg_color', 'mb_copyright_switch', 'on');
							...
							{
								$editor = Carbonick_Theme_Helper::get_option('copyright_editor');

								if ($this->mb_copyright_switch == 'on') {
									$editor = rwmb_meta('mb_copyright_editor');
								}
			?>
			<div class='copyright' <?php echo Carbonick_Theme_Helper::render_html($this->copyright_style()); ?>>
				<?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
				<div class='row' <?php echo Carbonick_Theme_Helper::render_html($this->copyright_spacing()); ?>>
