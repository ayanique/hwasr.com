...
*
*
* @class 		Carbonick_Breadcrumbs
* @version		1.0
* @category		Class
* @author 		WebGeniusLab
*/

if ( ! class_exists('Carbonick_Breadcrumbs') ) {
	class Carbonick_Breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__( 'Home', 'carbonick' ); // text for the 'Home' link
			$this->text_category = esc_html__( 'Archive by Category "%s"', 'carbonick' ); // text for a category page
			$this->text_search   = esc_html__( 'Search Results for "%s" Query', 'carbonick' ); // text for a search results page
			$this->text_tag      = esc_html__( 'Posts Tagged "%s"', 'carbonick' ); // text for a tag page
			$this->text_author   = esc_html__( 'Articles Posted by %s', 'carbonick' ); // text for an author page
			$this->text_404      = esc_html__( 'Error 404', 'carbonick' ); // text for the 404 page
			$this->text_page     = esc_html__( 'Page %s', 'carbonick' ); // text 'Page N'
			$this->text_cpage    = esc_html__( 'Comment Page %s', 'carbonick' ); // text 'Comment Page N'		
		}
			...
			if ( is_home() || is_front_page() ) {
				...
				// custom post type
					...
					if(function_exists('carbonick_woocommerce_breadcrumb')){
						...
						$args['delimiter'] = $sep;
						if ($show_home_link) $out .= $sep;
						$out .= carbonick_woocommerce_breadcrumb($args);
					}
				}
				$out .= $wrap_after;
			}

			echo Carbonick_Theme_Helper::render_html($out);
		}
	}
	new Carbonick_Breadcrumbs();
}