...
if (!$wgl_blog_atts) {
	...
	$wgl_blog_atts = [
		...
		'hide_media' => Carbonick_Theme_Helper::get_option('blog_list_hide_media'),
		'hide_content' => Carbonick_Theme_Helper::get_option('blog_list_hide_content'),
		'hide_blog_title' => Carbonick_Theme_Helper::get_option('blog_list_hide_title'),
		'hide_postmeta' => Carbonick_Theme_Helper::get_option('blog_list_meta'),
		'meta_author' => Carbonick_Theme_Helper::get_option('blog_list_meta_author'),
		'meta_comments' => Carbonick_Theme_Helper::get_option('blog_list_meta_comments'),
		'meta_categories' => Carbonick_Theme_Helper::get_option('blog_list_meta_categories'),
		'meta_date' => Carbonick_Theme_Helper::get_option('blog_list_meta_date'),
		'hide_likes' => !Carbonick_Theme_Helper::get_option('blog_list_likes'),
		'hide_share' => !Carbonick_Theme_Helper::get_option('blog_list_share'),
		'read_more_hide' => Carbonick_Theme_Helper::get_option('blog_list_read_more'),
...
if ($crop_square_img && $blog_columns !== '12') {
	switch($blog_columns) {
		case '6':
			$image_size = 'carbonick-800-700';
			break;
		default:
			$image_size = 'carbonick-700-700';
			break;
...
while ($query->have_posts()) : $query->the_post();
	...
	$meta_info_args['comments'] = !$meta_comments;
	...
	// Media
	if (!$hide_media) {
		$single->render_featured(true, $image_size, true, $hide_postmeta, $meta_cats);
	}
	...
	if(!$hide_likes || !$meta_comments || !$hide_share && function_exists('wgl_theme_helper')):
	...
	wp_link_pages(
		[
			'before' => '<div class="page-link">' . esc_html__('Pages', 'carbonick') . ': ',
			'after' => '</div>'
		]
	);
