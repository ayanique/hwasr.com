...
/**
* Carbonick Theme Helper
*
*
* @class        Carbonick_Theme_Helper
* @version      1.0
* @category     Class
* @author       WebGeniusLab
*/
...
if (! class_exists('Carbonick_Theme_Helper')) {
	class Carbonick_Theme_Helper
	{
        ...
        public static function get_option($name) { 
            if (  class_exists( 'Redux' ) && class_exists( 'Carbonick_Core_Public' ) ) {

                // Customizer
                if (!empty($GLOBALS['carbonick_set']) && $GLOBALS['carbonick_set'] != NULL) {
                    $theme_options = $GLOBALS['carbonick_set'];
                } else {
                    $theme_options = get_option( 'carbonick_set' );
                }

                if (empty($theme_options)) {
                    $theme_options = get_option( 'carbonick_default_options' );
                }

                return isset($theme_options[$name]) ? $theme_options[$name] : null;  

            }else{
                $default_option = get_option( 'carbonick_default_options' );
		...
		{
			$image = Carbonick_Theme_Helper::get_option($name."_bg_image");
		...
		{
			$name_load_more = ! empty($name_load_more) ? $name_load_more : esc_html__( 'Load More', 'carbonick' );
		...
		public static function render_sidebars($args = 'page')
		{
			...
			$carbonick_core = class_exists('Carbonick_Core');  
			...
			if (function_exists('is_shop') && is_shop() && ! $carbonick_core) {
		...
		public static function posted_meta_on()
		{
			...
			printf('<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__( 'Published in', 'carbonick' ) . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
			...
			if ($metadata) {
				printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s" title="%2$s">%1$s %3$s &times; %4$s</a></span>',
					esc_html_x( 'Full size', 'Used before full size attachment link.', 'carbonick' ),
			...
			edit_post_link(
				sprintf(
					...
					wp_kses( __( 'Edit<span class="screen-reader-text"> "%s"</span>', 'carbonick' ), $allowed_html ) ,
						get_the_title()
