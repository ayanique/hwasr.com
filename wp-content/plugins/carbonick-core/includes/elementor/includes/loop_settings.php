...
if (! class_exists('Wgl_Loop_Settings')) {
	class Wgl_Loop_Settings
	{
		...
		public static function cache_query($args = [])
		{
			...
			$custom_query = wp_cache_get($cache_key, 'carbonick_theme_cache');
			if (false === $custom_query) {
				$custom_query = new \WP_Query($args);
				if (!is_wp_error($custom_query) && $custom_query->have_posts()) {

					wp_cache_set($cache_key, $custom_query, 'carbonick_theme_cache');
				}
			}
			return $custom_query;
		}
		...
		public static function init($self, $atts = [])
		{
			...
			$self->start_controls_section(
				'query_section',
				[
					'label' => esc_html__( 'Query', 'carbonick-core' ),
			...
			$self->add_control(
				'number_of_posts',
				[
					'label' => esc_html__( 'Post count', 'carbonick-core' ),
			...
			$self->add_control(
				'order_by',
				[
					'label' => esc_html__( 'Order by', 'carbonick-core' ),
					'type' => Controls_Manager::SELECT,
					'default' => 'date',
					'options' => [
						'date' => esc_html__( 'Date', 'carbonick-core' ),
						'title' => esc_html__( 'Title', 'carbonick-core' ),
						'author' => esc_html__( 'Author', 'carbonick-core' ),
						'modified' => esc_html__( 'Modified', 'carbonick-core' ),
						'rand' => esc_html__( 'Random', 'carbonick-core' ),
						'comment_count' => esc_html__( 'Comments', 'carbonick-core' ),
						'menu_order' => esc_html__( 'Menu Order', 'carbonick-core' ),
					],
				]
			);
			...
			$self->add_control(
				'order',
				[
					'label' => esc_html__( 'Order', 'carbonick-core' ),
					'type' => Controls_Manager::SELECT,
					'options' => [
						'DESC' => esc_html__( 'Descending', 'carbonick-core' ),
						'ASC' => esc_html__( 'Ascending', 'carbonick-core' ),
					],
					'default' => 'DESC',
				]
			);
			...
			if (! isset($atts['hide_cats'])) {
				...
				$self->add_control(
					'categories',
					[
						'label' => esc_html__( 'Filter By Category Slug', 'carbonick-core' ),
				...
				$self->add_control(
					'exclude_categories',
					[
						'label' => esc_html__( 'Exclude These Categories', 'carbonick-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Leave empty for all', 'carbonick-core' ),
					]
				);  
			}           
			...
			if (! isset($atts['hide_tags']) ) :
				...
				$self->add_control(
					'tags',
					[
						'label' => esc_html__( 'Filter By Tags Slug', 'carbonick-core' ),
						'type' => Controls_Manager::SELECT2,
						'multiple' => true,
						'label_block' => true,
				...
				$self->add_control(
					'exclude_tags',
					[
						'label' => esc_html__( 'Exclude These Tags', 'carbonick-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Leave empty for all', 'carbonick-core' ),
					]
				);
			...
			$self->add_control(
				'taxonomies',
				[
					'label' => esc_html__( 'Taxonomies', 'carbonick-core' ),
					'type' => Controls_Manager::SELECT2, 
					'multiple' => true,
					'label_block' => true,
			...
			$self->add_control(
				'exclude_taxonomies',
				[
					'label' => esc_html__( 'Exclude These Taxonomies', 'carbonick-core' ),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'On', 'carbonick-core' ),
					'label_off' => esc_html__( 'Off', 'carbonick-core' ),
					'description' => esc_html__( 'Filter output by custom taxonomies categories, enter category names here.', 'carbonick-core' ),
				]
			);
			...
			if (! isset($atts['hide_individual_posts'])) {
				...
				$self->add_control(
					'by_posts',
					[
						'label' => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'carbonick-core' ),
						'type' => Controls_Manager::SELECT2,
						'multiple' => true,
						'label_block' => true,
				...
				$self->add_control(
					'exclude_any',
					[
						'label' => esc_html__( 'Exclude These Posts/Pages/Custom Post Types', 'carbonick-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'carbonick-core' ),
					]
				);
			}
			...
			$self->add_control(
				'author',
				[
					'label' => esc_html__( 'Author', 'carbonick-core' ),
					'type' => Controls_Manager::SELECT2,
					'multiple' => true,
					'label_block' => true,
			...
			$self->add_control(
				'exclude_author',
				[
					'label' => esc_html__( 'Exclude Author', 'carbonick-core' ),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'On', 'carbonick-core' ),
					'label_off' => esc_html__( 'Off', 'carbonick-core' ),
					'description' => esc_html__( 'Filter by author name', 'carbonick-core' ),
				]
			);
		...
		public static function categories_suggester() {
			...
			foreach (get_categories() as $cat) {
				...
				$content[(string) $cat->slug] = $cat->cat_name.(! empty($parent) ? esc_html__( ' (Parent categories: (', 'carbonick-core') .$parent.'))' : "");
			}
		...
		public static function taxonomies_suggester($atts)
		{
			...
			foreach ($result as $tag) {
				...
				$content[ $tag->taxonomy.":".$tag->slug ] = $tag->name . ' (' . $tag->taxonomy . ')' . (! empty($parent) ? esc_html__( ' (Parent categories: (', 'carbonick-core') . $parent . '))' : '');
			}
			return $content;
		}
