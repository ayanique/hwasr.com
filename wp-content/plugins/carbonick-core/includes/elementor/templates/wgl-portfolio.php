...
use WglAddons\Widgets\Wgl_Double_Headings;
...
class WglPortfolio
{
    ...
    public function render($atts, $self = false)
    {
        ...
        ob_start();
            if(!empty($self)){
               Wgl_Double_Headings::init_double_headings($self);
            }
        ...
        if ((bool) $show_filter || !empty($double_headings)) {

            $out .= '<div class="wgl-filter_wrapper">';
        }
        ...
        if ((bool) $show_filter) {
            ...
            $filter_class = $portfolio_layout != 'carousel' ? 'isotope-filter' : 'carousel-filter';
            $filter_class .= ' filter-' . $filter_align;
            $out .= '<div class="wgl-filter portfolio__filter ' . esc_attr($filter_class) . '">';
            $out .= $this->getCategories($query_args, $query_results);
            $out .= '</div>';
        }

        if ($portfolio_layout == 'carousel' && (bool) $use_prev_next && $prev_next_position == 'top_right') {
            $out .= '<div class="carousel_arrows"><span class="left_slick_arrow"><span></span></span><span class="right_slick_arrow"><span></span></span></div>';
        }
        ...
        $out .= $this->output_loop_query($query_results, $atts, $self);
        $out .= '</div>';
        $out .= '</div>';
        ...
        if ($navigation == 'pagination') {
            ...
            $out .= \Carbonick_Theme_Helper::pagination('10', $query_results, $nav_align);
        }
    ...
    public function output_loop_query($q, $params, $self = false)
    {
        extract($params);
        $out = '';
        $count = 0;
        ...
        if ($q->have_posts()) :
            ...
            while ($q->have_posts()) :
                ...
                if ($portfolio_layout == 'masonry4' && $first_title === 0 && $show_first_title === 'yes') {
                    echo '<article class="first_title wgl-portfolio-list_item item wgl_col-6" ' . $style_gap . '>',
                        '<div class="wgl-portfolio-item_wrapper_bg" ' . $style_gap_inner . '>',
                        '<div class="wgl-portfolio-item_wrapper">',
                        (!empty($first_title_sub_title) ? '<div class="subtitle">' . $first_title_sub_title . '</div>' : ''),
                        (!empty($first_title_title) ? '<h4 class="title">' . $first_title_title . '</h4>' : ''),
                        '</div>',
                        '</div>',
                        '</article>';
    ...
    public function wgl_portfolio_carousel_item($params, $item_class, $return)
    {
        ...
        $wrap_class .= $freeScroll ? ' freeScroll' : '';

        if(!empty($freeScroll)){
            $posts_per_row = $c_slide_per_single = 1;
        }
    ...
    private function post_content($params)
    {
        ...
        $content = \Carbonick_Theme_Helper::modifier_character($content, $chars_count, "");
    ...
    public function render_icon_link($params, $icon_out)
    {
        extract($params);
        ...
        switch ($img_click_action) {
            case 'popup':
                $link = "<a href='" . $wp_get_attachment_url . "' class='swipebox' data-elementor-open-lightbox='no'>" . $icon_out . "</a>";
                break;

            case 'single':
                $link = "<a href='" . get_permalink() . "' class='single_link'>" . $icon_out . "</a>";
                break;
            ...
            case 'custom':
                if (rwmb_meta('mb_portfolio_link') == 1) {
                    $mb_custom_url = !empty(rwmb_meta('portfolio_custom_url')) ? rwmb_meta('portfolio_custom_url') : get_permalink();
                    $mb_custom_url_target = !empty(rwmb_meta('portfolio_custom_url_target')) ? '_blank' : '_self';
                    $link = "<a href='" . esc_url($mb_custom_url) . "' target=" . esc_attr($mb_custom_url_target) . " class='custom_link'>" . $icon_out . "</a>";
                }
                break;
        }
    ...
    public function gallery_mode_enabled($params, $link, $icon_out)
    {
        ...
        if (!empty($icon_out)) {
            ...
            if ($info_position != 'under_image' && $image_anim != 'sub_layer') {
                ...
                echo $this->render_icon_link($params, $icon_out);
            }
    ...
    public function standard_mode_enabled($params, $link, $post_meta, $icon_out)
    {
        ...
        } elseif ($img_click_action == 'custom' && rwmb_meta('mb_portfolio_link') == 1) {
        ...
        if ($info_position == 'inside_image' && $image_anim == 'simple' && $link_attr) {
    ...
    private function single_post_date()
    {
        if (rwmb_meta('mb_portfolio_single_meta_date') != 'default') {
            ...
            $date_enable = \Carbonick_Theme_Helper::get_option('portfolio_single_meta_date');
        }

        if ($date_enable) {
            return '<span class="post_date">' .
                '<i class="post_date-icon far fa-calendar-alt"></i>' .
                get_the_time('F') .
                ' ' .
                get_the_time('d') .
                '</span>';
    ...
    {
        $show_likes = \Carbonick_Theme_Helper::get_option('portfolio_single_meta_likes');
    ...
    {
        $author = \Carbonick_Theme_Helper::get_option('portfolio_single_meta_author');
        if (!empty($author)) {
            return '<span class="post_author">' .
                '<i class="post_author-icon flaticon-user"></i>' .
                esc_html__('by ', 'carbonick-core') .
    ...
    {
        $comments_enabled = \Carbonick_Theme_Helper::get_option('portfolio_single_meta_comments');

        if ($comments_enabled) {
            $comments_num = get_comments_number(get_the_ID());

            return '<span class="post_comments">' .
                '<a href="' . esc_url(get_comments_link()) . '">' .
                '<i class="post_comments-icon flaticon-chat-1"></i>' .
                esc_html($comments_num) .
                ' ' .
                esc_html(_n('Comment', 'Comments', $comments_num, 'carbonick-core')) .
                '</a>' .
                '</span>';
        }
    }
    ...
    private function single_post_cats()
    {
        if (class_exists('RWMB_Loader') && rwmb_meta('mb_portfolio_single_meta_categories') != 'default') {
            ...
            $cats_enable = \Carbonick_Theme_Helper::get_option('portfolio_single_meta_categories');
        }
    ...
    public function wgl_portfolio_single_item($parameters, $item_class = '')
    {
        ...
        if (class_exists('RWMB_Loader')) {
            ...
            $featured_image = \Carbonick_Theme_Helper::options_compare('portfolio_featured_image_type', 'mb_portfolio_featured_image_conditional', 'custom');
            if ($featured_image == 'replace') {
                $featured_image_replace = \Carbonick_Theme_Helper::options_compare('portfolio_featured_image_replace', 'mb_portfolio_featured_image_conditional', 'custom');
            }
            ...
            $mb_hide_all_meta = \Carbonick_Theme_Helper::get_option('portfolio_single_meta');

            if (rwmb_meta('mb_portfolio_above_content_cats') == 'default') {
                $tags_enable = \Carbonick_Theme_Helper::get_option('portfolio_above_content_cats');
            } else {
            ...
            if (rwmb_meta('mb_portfolio_above_content_share') != 'default') {
                ...
                $shares_enable = \Carbonick_Theme_Helper::get_option('portfolio_above_content_share');
            }
        }

        $single_post_align = \Carbonick_Theme_Helper::options_compare('portfolio_single_align', 'mb_portfolio_post_conditional', 'custom');
        ...
        if ($mb_editor || $portfolio_info || $social_share) {
            ...
            if ($social_share) {
                $portfolio_info .= '<div class="portfolio__custom-meta meta_shares">' . $social_share . '</div>';
            }
        ...
        if (strlen($wp_get_attachment_url)) {
            if ($params['portfolio_layout'] == 'masonry2') :
                ...
                if ($params['portfolio_layout'] == 'carousel' && !empty($params['variable_width'])) {
                    $wgl_featured_image_url = aq_resize($wp_get_attachment_url, '800', '520', $crop, true, true);
                } else {
                    switch ($params['posts_per_row']) { // ratio 0.7872
                        ...
                        case '2':
                            ...
                            break;
                        case '3':
                            $wgl_featured_image_url = aq_resize($wp_get_attachment_url, '740', '700', $crop, true, true);
                            break;
                        case '4':
                            $wgl_featured_image_url = aq_resize($wp_get_attachment_url, '570', '530', $crop, true, true);
                            break;
                        case '5':
                            $wgl_featured_image_url = aq_resize($wp_get_attachment_url, '570', '530', $crop, true, true);
                            break;
    ...
    {
        if (null === $before) $before = __('Tags: ', 'carbonick-core');
    ...
    public function getCategories($params, $query)
    {
        ...
        $out = '<a href="#" data-filter=".item" class="active">' . esc_html__('All', 'carbonick-core') . '<span class="number_filter"></span></a>';
