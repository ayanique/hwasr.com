...
class WglTeam
{
	...
	public function render($atts, $content = null)
	{
		...
		if ($use_carousel) {
			...
			$carousel_options = [
				...
				'resp_medium' => ( $resp_medium ? $resp_medium : '1170'),
				'resp_medium_slides' => $resp_medium_slides,
				'resp_tablets' => ( $resp_tablets ? $resp_tablets : '1025'),
				'resp_tablets_slides' => $resp_tablets_slides,
				'resp_mobile' => ( $resp_mobile ? $resp_mobile : '768'),
			...
			<div class="team-items_wrap">
				...
				switch ($use_carousel) {
					case true:
						...
						break;
					default:
						echo \Carbonick_Theme_Helper::render_html($team_items);
						break;
				}
				?>
			</div>
	...
	public function render_wgl_team($atts)
	{
		...
		switch ($posts_per_line) { // ratio = 0.96
			...
			case '3': $team_image_dims = ['width' => '720', 'height' => '700'];  break;
			case '4':
			case '5':
			case '6': $team_image_dims = ['width' => '540', 'height' => '640'];  break;
		}
	...
	public function render_wgl_team_item($single_member = false, $item_atts, $team_image_dims, $counter)
	{
		...
		extract($item_atts);
		$team_info = $team_icons = $featured_image = $team_title = $team_icons_wrap = $link_info ='';
		$id = get_the_ID();
		$post = get_post($id);
		$link_to = get_permalink($id);
		$name_department = get_post_meta($id, 'name_department', true);
		...
		if (! $hide_soc_icons && $team_icons) {
			$team_icons_wrap  = '<div class="team__icons">';
				$team_icons_wrap .= !$single_member ? '<span class="team-icon team-icon-share"></span>' : '';
		...
		if (! $hide_title) {
			$team_title = '<h2 class="team-title">';
				$team_title .= $single_link_heading && ! $single_member &&  $info_position !== 'inside_image' ? '<a href="'.esc_url($link_to).'">' : '';
					$team_title .= get_the_title();
				$team_title .= $single_link_heading && ! $single_member &&  $info_position !== 'inside_image' ? '</a>' : '';
			$team_title .= '</h2>';
		}
		...
		if (! $single_member && ! $hide_content) {
			...
			$excerpt = \Carbonick_Theme_Helper::modifier_character($excerpt, $letter_count, "");
		}

		// Inside Info Link
		$link_info = $single_link_heading && ! $single_member &&  $info_position == 'inside_image' ? '<a class="team-link_inside" href="'.esc_url($link_to).'"><i class="team-link__icon flaticon-arrow"></i></a>' : '';
		...
		if (! $single_member) :
			...
			echo '<div class="team-single_wrapper"', ($info_bg_url ? ' style="background-image: url('.esc_url($info_bg_url).')"' : ''), '>';
				...
				echo '<div class="team-info_wrapper">',
					$team_title,
					$department ? '<div class="team-info_item team-department"><h5>'.esc_html($name_department).'</h5><span>'.esc_html($department).'</span></div>' : '',
					$team_info,
					$team_icons_wrap,
				'</div>';
			echo '</div>';
