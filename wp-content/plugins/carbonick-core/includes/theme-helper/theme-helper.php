...
class WglThemeHelper
{
	...
	public function render_post_share($show_share)
	{
		...
		if ($show_share) : ?>
			...
			<div class="share_social-wpapper">
				<span class="share_title"><?php echo apply_filters('carbonick_title_share', esc_html__('Share:', 'carbonick-core') );?></span>
				<a class="share_link share_twitter" target="_blank" href="<?php echo esc_url('https://twitter.com/intent/tweet?text='. get_the_title() .'&amp;url='. get_permalink()); ?>"><span class="fab fa-twitter"></span><span class="share_title"><?php esc_html_e('Twitter', 'carbonick-core'); ?></span></a>
				<a class="share_link share_facebook" target="_blank" href="<?php echo esc_url('https://www.facebook.com/share.php?u='. get_permalink()); ?>"><span class="fab fa-facebook-f"></span><span class="share_title"><?php esc_html_e('Facebook', 'carbonick-core'); ?></span></a>
				<?php
					if (strlen($img_url[0]) > 0) {
						echo '<a class="share_link share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url[0]) .'"><span class="fab fa-pinterest-p"></span><span class="share_title">'.esc_html__('Pinterest', 'carbonick-core').'</span></a>';
					}
				?>
				<a class="share_link share_linkedin" href="<?php echo esc_url('http://www.linkedin.com/shareArticle?mini=true&url='.substr(urlencode( get_permalink() ),0,1024));?>&title=<?php echo esc_attr(substr(urlencode(html_entity_decode(get_the_title())),0,200));?>" target="_blank" ><span class="fab fa-linkedin-in"></span><span class="share_title"><?php esc_html_e('LinkedIn', 'carbonick-core'); ?></span></a>
			</div>
			<!-- //post share block -->
			<?php
		endif;
		...
		<div class="share_post-container">
			<div class="share_social-wpapper">
				<ul>
					...
					if (strlen($img_url[0]) > 0) {
						echo '<li>',
							'<a class="share_post share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url[0]) .'"><span class="fab fa-pinterest"></span></a>',
						'</li>';
					}
					?>
					<li>
	...
	{
		$description = esc_html__( 'Socials', 'carbonick-core' );

		$facebook  = Carbonick_Theme_Helper::options_compare('soc_icon_facebook', 'mb_customize_soc_shares', 'on');
		$twitter   =  Carbonick_Theme_Helper::options_compare('soc_icon_twitter', 'mb_customize_soc_shares', 'on');
		$linkedin  = Carbonick_Theme_Helper::options_compare('soc_icon_linkedin', 'mb_customize_soc_shares', 'on');
		$pinterest = Carbonick_Theme_Helper::options_compare('soc_icon_pinterest', 'mb_customize_soc_shares', 'on');
		$tumblr    = Carbonick_Theme_Helper::options_compare('soc_icon_tumblr', 'mb_customize_soc_shares', 'on');

		$fixed  = Carbonick_Theme_Helper::options_compare('soc_icon_position', 'mb_customize_soc_shares', 'on');
		$offset = Carbonick_Theme_Helper::get_option('soc_icon_offset');
		...
		$share = Carbonick_Theme_Helper::options_compare('soc_icon_style', 'mb_customize_soc_shares', 'on');
				...
				if ($share == 'hovered' ) : ?>
					<div class="share_social-desc">
						<div class="share_social-title">
							<?php
							echo apply_filters('carbonick_desc_socials', $description);
							?>
						</div>
				...
				<ul>
					...
					if (! empty($share_pinterest) ) :
						...
						if (strlen($img_url[0]) > 0) {
							echo
							  '<li>',
								'<a class="share_page share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url[0]) .'"><span class="fab fa-pinterest-p"></span></a>',
							  '</li>'
							;
						}
					endif;
					...
					$custom_share = Carbonick_Theme_Helper::get_option('add_custom_share');
					if (isset($custom_share) && ! empty($custom_share)) {
						for ( $i = 1; $i <= 10; $i++) {
							${'custom_share_'.$i} = Carbonick_Theme_Helper::get_option('select_custom_share_text-'.$i);
							${'custom_share_icons'.$i} = Carbonick_Theme_Helper::get_option('select_custom_share_icons-'.$i);
