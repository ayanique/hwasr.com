...
{ 
    ?>
    <h3><?php esc_html_e( 'Social media accounts', 'carbonick-core' ); ?></h3>

    <table class="form-table">
        <?php
        foreach ( wgl_user_social_medias_arr() as $social => $value) { ?>
            <tr>
                <th><label for="<?php echo esc_attr($social); ?>" style="text-transform: capitalize;"><?php esc_html_e( $value, 'carbonick-core' ); ?></label></th>
                <td>
                    ...
                    <span class="description"><?php esc_html_e( 'Your '.$value.' url.', 'carbonick-core' ); ?></span>
                </td>
            </tr>
            <?php
        } ?>
...
function wgl_wp_head_custom_code()
{
    ...
    if (class_exists('Carbonick_Theme_Helper')) {
        $header_custom_code = Carbonick_Theme_Helper::get_option('header_custom_js');
    }
...
{
    if (class_exists('Carbonick_Theme_Helper')){
        $custom_js = Carbonick_Theme_Helper::get_option('custom_js');
    }
