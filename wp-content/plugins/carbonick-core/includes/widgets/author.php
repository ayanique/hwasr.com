...
class Author extends WP_Widget
{
	...
	function __construct()
	{
		parent::__construct(
			...
			esc_html__( 'WGL Blog Author', 'carbonick-core' ), // Name
			array( 'description' => esc_html__( 'WGL Widget ', 'carbonick-core' ), ) // Args
		);
	...
	public function enqueue_backend_scripts()
	{
		...
		wp_enqueue_script('carbonick-upload-img', get_template_directory_uri() . '/core/admin/js/img_upload.js', array('jquery'), false, true);
	}
	...
	public function widget($args, $instance)
	{
		...
		if ('' === get_post_meta($attachment_id, '_wp_attachment_image_alt', true)) {
			$alt = the_title_attribute(array('before' => esc_html__('Featured author image: ', 'carbonick-core'), 'echo' => false));
		} else {
		...
		if ('' === get_post_meta($attachment_id_s, '_wp_attachment_image_alt', true)) {
			$alt_s = the_title_attribute(array('before' => esc_html__('Featured author signature: ', 'carbonick-core'), 'echo' => false));
		} else {
		...
		$widgetClasses = 'carbonick_author-widget widget carbonick_widget';
	...
	public function form($instance)
	{
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $title_name ) ); ?>"><?php esc_html_e( 'Title:', 'carbonick-core' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $author_name ) ); ?>"><?php esc_html_e( 'Author Name:', 'carbonick-core' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>"><?php esc_html_e( 'Text:', 'carbonick-core' ); ?></label>
		...
		<p>
		  <label for="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>"><?php esc_html_e( 'Author Image:', 'carbonick-core' ); ?></label><br />
			<img class="carbonick_media_image" src="<?php if (! empty($instance[$image_name])) {echo esc_url( $instance[$image_name] );} ?>" style="max-width: 100%" />
			<input type="text" class="widefat carbonick_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>" value="<?php echo esc_attr( $image ); ?>">
			<a href="#" class="button carbonick_media_upload"><?php esc_html_e('Upload', 'carbonick-core'); ?></a>
		</p>        

		<p>
		  <label for="<?php echo esc_attr( $this->get_field_id($bg_image) ); ?>"><?php esc_html_e( 'Background Image:', 'carbonick-core' ); ?></label><br />
			<img class="carbonick_media_image" src="<?php if (! empty($instance[$bg_image])) {echo esc_url( $instance[$bg_image] );} ?>" style="max-width: 100%" />
			<input type="text" class="widefat carbonick_media_url" name="<?php echo esc_attr( $this->get_field_name($bg_image) ); ?>" id="<?php echo esc_attr( $this->get_field_id($bg_image) ); ?>" value="<?php echo esc_attr( $bg ); ?>">
			<a href="#" class="button carbonick_media_upload"><?php esc_html_e('Upload', 'carbonick-core'); ?></a>
		</p>        

		<p>
		  <label for="<?php echo esc_attr( $this->get_field_id($image_signature) ); ?>"><?php esc_html_e( 'Author Signature:', 'carbonick-core' ); ?></label><br />
			<img class="carbonick_media_image" src="<?php if (! empty($instance[$image_signature])) {echo esc_url( $instance[$image_signature] );} ?>" style="max-width: 100%" />
			<input type="text" class="widefat carbonick_media_url" name="<?php echo esc_attr( $this->get_field_name($image_signature) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_signature) ); ?>" value="<?php echo esc_attr( $signature ); ?>">
			<a href="#" class="button carbonick_media_upload"><?php esc_html_e('Upload', 'carbonick-core'); ?></a>
		</p>
