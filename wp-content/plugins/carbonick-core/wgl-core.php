...
class Carbonick_CorePlugin {
    ...
    static function activation_check() {
        if ( ! self::compatible_version() ) {
            ...
            wp_die( __( '"Carbonick-core" Plugin compatible with "Carbonick" theme only!', 'carbonick-core' ) );
        }
    }
    ...
    function disabled_notice() {
       echo '<strong>' , esc_html__( '"Carbonick-core" Plugin compatible with "Carbonick" theme only!', 'carbonick-core' ) , '</strong>';
    } 
...
new Carbonick_CorePlugin();

register_activation_hook( __FILE__, array( 'Carbonick_CorePlugin', 'activation_check' ) );
 ...
 */
function deactivate_carbonick_core() {
    require_once plugin_dir_path( __FILE__ ) . 'includes/class-wgl-core-deactivator.php';
    Carbonick_Core_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_carbonick_core' );
register_deactivation_hook( __FILE__, 'deactivate_carbonick_core' );
 ...
 * @since    1.0.0
 */
function run_carbonick_core() {

    $plugin = new Carbonick_Core();
    $plugin->run();
